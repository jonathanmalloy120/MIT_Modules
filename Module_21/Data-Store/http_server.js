...
app.get('/published/:boolean', function(req, res){
    ...
    // YOUR CODE
    db.get('posts').filter({ published: req.params.boolean }).value()
    console.log(db.get('posts').filter({ published: req.params.boolean }).value())
    res.send(db.get('posts').filter({ published: req.params.boolean }).value())
    //looking for string "true" or "false" right now, would have to alter to look for boolean values
});
...
app.get('/delete/:id/', function(req, res){
    ...
    // YOUR CODE
    db.get('posts').filter({  id: req.params.id}).value()
    console.log(db.get('posts').filter({  id: req.params.id}).value())
